"""Cron persistence + execution-state tests.

Covers JSONL round-trip of ``CronJob`` (timestamps, durations, schedule
discriminator) and the post-fire bookkeeping done by ``CronStore.executed``.
"""
...
@pytest.mark.asyncio
async def test_execute_job_records_last_run_at(tmp_path) -> None:
    bus = MessageBus()
    address = _address()
    ...
    async with CronStore(tool._store_path) as store:
        ...
        # New format keeps only last_run_at — no last_status / last_error.
