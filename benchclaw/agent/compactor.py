...
``SummaryEvent`` backed by recent activity-log entries. The compactor
itself does not call the LLM — that's the simple, deterministic story
BenchClaw ships today.
"""
...
class Compactor:
    ...
    def __init__(self, agent_config: AgentConfig, log_store: LogStore | None) -> None:
    ...
    def maybe_compact(
        ...
        session.compact(self.log_store)
        logger.warning(
